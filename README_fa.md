...
```
دانلود و نصب خودکار Mimic (سمت سرور)، نیازی به دانلود دستی نیست
amd64/arm64؛ در صورت خطا خودکار غیرفعال می‌شود؛ کلاینت همچنان نیاز به نصب دستی دارد
```
...
```
افزودن پشتیبانی Mimic (جعل TCP) (hysteria v2.12.0+)، به‌صورت پیش‌فرض خاموش
با فعال‌سازی، پرش پورت رد می‌شود؛ کلاینت باید لینوکس با root باشد (مثل روتر نرم‌افزاری)
mihomo/sing-box از mimic پشتیبانی نمی‌کنند — فقط پیکربندی بومی
```


[حالت Realm سوراخ‌کردن P2P — استفاده از Hysteria2 پشت NAT، حتی رله از طریق Cloudflare](md/fa/realm.md)
...
* پشتیبانی Mimic (جعل TCP، hysteria v2.12.0+): پیش‌فرض خاموش؛ هر دو طرف نیاز به لینوکس+root و ماژول mimic؛ ناسازگار با پرش پورت
...
#### ۱. [راهنمای فایروال](md/fa/firewall.md)

#### ۲. [گواهی‌های خودامضا](md/fa/certificate.md)

#### ۳. [لیست سیاه ISPهای محدودکننده UDP (به‌روزرسانی ۲۰۲۵/۰۱/۰۷)](md/fa/blacklist.md)

#### ۴. [نحوه تنظیم تأخیر / سرعت آپلود / دانلود](md/fa/speed.md)

#### ۵. [کلاینت‌های پشتیبانی‌شده](md/fa/client.md)

#### ۶. [سوالات متداول / عیب‌یابی](md/fa/issues.md)

#### ۷. [فعال‌سازی وبسایت نقاب‌دار](md/fa/masquerade.md)

#### ۸. [حالت Realm — سوراخ‌کردن P2P](md/fa/realm.md)
