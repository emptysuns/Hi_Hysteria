...
```
بازنویسی ماژولار؛ پشتیبانی چندزبانه i18n (en/zh/fa/ru)؛ رفع اشکالات پیکربندی mihomo (ازدحام/gecko/hop-interval/Realm) + تغییر نام ClashMeta→mihomo؛ تولید پیکربندی کلاینت sing-box جدید
```
...
> Hysteria2 یک ابزار شبکه قدرتمند بهینه‌سازی شده برای محیط‌های شبکه نامساعد (شتاب‌دهی دوطرفه) است، مانند شبکه‌های ماهواره‌ای، Wi-Fi عمومی شلوغ، **اتصال به سرورهای خارج از چین** و غیره. مبتنی بر پروتکل QUIC اصلاح‌شده.
> 
它很好的解决了在搭建富强魔法服务器时最大的痛点——**线路拉跨**。

۱. اتصال مستقیم به دیتاسنتر NTT ژاپن + Cloudflare WARP، بدون بهینه‌سازی مسیر 163، تست سرعت در ساعات اوج ۲۰:۰۰-۲۳:۰۰:

~~ماشین تست یک کانتینر LXC است — CPU به دلیل محدودیت‌های عملکردی کاملاً اشباع شده~~

![image](imgs/speed.png)

۲. بدون مسیریابی بهینه‌شده برای چین، دیتاسنتر ShockHosting لس‌آنجلس، 1c128m OVZ NAT، 4K@60fps:
...
ممکن است باگ‌هایی وجود داشته باشد — لطفاً در صورت مشاهده issue ثبت کنید. ستاره‌ها welcome! ⭐ شما انگیزه نگهداری است.
...
* تولید کد QR برای لینک‌های اشتراک‌گذاری hy2 مستقیماً در ترمینال
* تولید فایل‌های پیکربندی اصلی کلاینت Hysteria2 با پارامترهای کامل
* راه‌اندازی فرآیند با اولویت بالا برای Hysteria2 (اولویت سرعت)
* پرش پورت و دیمن Hysteria2 از طریق اسکریپت‌های شروع خودکار مدیریت می‌شوند
* اسکریپت نصب Hysteria v1 به عنوان یک گزینه حفظ شده است
* محاسبه BDP (حاصل‌ضرب پهنای‌باند-تأخیر) برای تنظیم پارامترهای QUIC
...
* Realm از طریق Cloudflare WARP — اتصال به Hysteria2 از طریق IP WARP، مخفی‌سازی IP واقعی سرور
...
#### ۲. [گواهی خودامضا](md/certificate.md)
...
**اسکریپت ممکن است با هر به‌روزرسانی تغییر کند. لطفاً فرآیند نمایشی را باز کرده و با دقت بررسی کنید تا از خطاهای غیرضروری جلوگیری شود!**

<details>
  <summary>نمایش طولانی، برای مشاهده کلیک کنید</summary>
<pre><blockcode> 
...
**اگر پیشنهادی برای ویژگی جدید دارید، لطفاً issue ثبت کنید! PR برای افزودن به برنامه‌ها یا اصلاح کد من welcome است.**
...
Hysteria2 در محیط‌های شبکه با تأخیر بالا و اتلاف بسته بالا عملکرد خوبی دارد، به لطف الگوریتم کنترل ازدحام Brutal اختصاصی خود.

این به تحقیق در مورد محیط‌های شبکه چالش‌برانگیز کمک می‌کند. هدف این مخزن کمک به محققان برای پیکربندی آسان Hysteria2 است. در اصل، ما از پیکربندی سفارشی برای تمام ویژگی‌های Hysteria2 پشتیبانی می‌کنیم.
