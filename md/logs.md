...
4、修复:流量统计(菜单13)行数据不显示——行模板占位符被 i18n() 提前消费(`printf "$(i18n key)" args` 模式下内层 printf 无参展开,流量/在线用户行永远输出空字段、设备数恒为 0);现改为参数直接传入 i18n。解析改用 yq(兼容任意 JSON 格式,401/非 JSON 响应会显示具体错误而非静默空白);活动连接表格重设计:去掉恒为 "-" 的 Hooked-Req-Addr 列,Req-Addr 列正名为"目标地址",表头按终端显示宽度对齐(CJK 计 2 列)
```
