...
```
Modular refactor; i18n multi-language support (en/zh/fa/ru); mihomo config fixes (congestion/gecko/hop-interval/Realm) + ClashMeta→mihomo rename; new sing-box client config
```
...
Have questions or want to share tips? Join our Telegram group:
...
> Hysteria2 is a feature-rich network tool optimized for harsh network environments (dual-side acceleration), such as satellite networks, crowded public Wi-Fi, **connecting to overseas servers from China**, etc. Based on a modified QUIC protocol.
> 
它很好的解决了在搭建富强魔法服务器时最大的痛点——**线路拉跨**。

1. Direct connection to Japan NTT data center + Cloudflare WARP, unoptimized 163 backbone, 20:00-23:00 peak hour speedtest:

~~Test machine is an LXC container — CPU maxed out due to performance limits, can't push further~~

![image](imgs/speed.png)

2. No China-optimized routing, Los Angeles ShockHosting data center, 1c128m OVZ NAT, 4K@60fps:
...
The author assumes no risk or legal liability for any issues caused by this software. Please comply with the GPL open-source license.

There may be bugs — please file an issue if you encounter any. Stars welcome! Your ⭐ motivates maintenance.
...
* QR code generation for hy2 share links directly in the terminal
* Generate Hysteria2 original client config files with full client parameters
* High-priority process launch for Hysteria2 (speed-first)
* Port hopping and Hysteria2 daemon managed via auto-start scripts for better extensibility
...
**The script may change with each update. Please expand and carefully review the demo process to avoid unnecessary errors!**

<details>
  <summary>Long demo, click to view</summary>
<pre><blockcode> 
...
**If you have feature suggestions, please open an issue! PRs welcome to add todos or fix my spaghetti code.**
...
Hysteria2 performs well in high-latency, high-packet-loss network environments, thanks to its custom Brutal congestion control algorithm.
